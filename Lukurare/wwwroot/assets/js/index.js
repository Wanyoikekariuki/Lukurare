var MVVM = {
    ...
    init: function () {
        ...
        var viewModel = function () {
            ...
            self.whenPosted = ko.observable();
            self.whenAgo = ko.observable();
            this.ProjectName = ko.observable();
                ...
                var functionDone = function (hasError, message, data) {
                    ...
                    if (hasError) {
                        ...
                        data.Result.forEach(function (item) {
                            ...
                            var datePosted = new Date(item.DatePosted);
                            var now = new Date();

                            // Calculate the difference in milliseconds
                            var diff = now - datePosted;
                            ...
                            if (seconds < 60) {
                                timeDiff = seconds + (seconds === 1 ? " second ago" : " seconds ago");
                            } else if (minutes < 60) {
                                timeDiff = minutes + (minutes === 1 ? " minute ago" : " minutes ago");
                            } else if (hours < 24) {
                                timeDiff = hours + (hours === 1 ? " hour ago" : " hours ago");
                            } else if (days < 7) {
                                timeDiff = days + (days === 1 ? " day ago" : " days ago");
                            } else if (weeks < 4) {
                                timeDiff = weeks + (weeks === 1 ? " week ago" : " weeks ago");
                            } else if (months < 12) {
                                timeDiff = months + (months === 1 ? " month ago" : " months ago");
                            } else {
                                timeDiff = years + (years === 1 ? " year ago" : " years ago");
                            }
            ...
            this.indexSearch = function () {
                ...
                if (cPage !== '/Projects') {
                    // Redirect to the Projects page with search parameters
                    window.location.href = '/Projects?industry=' + encodeURIComponent(industry) + '&location=' + encodeURIComponent(location);
                } else {
                    // If the current page is 'Projects', you can call your search function here
            ...
            //    // Your logic for searching projects
            //    // Example: Redirect to the Projects page with search parameters
            //    window.location.href = '/Projects?industry=' + encodeURIComponent(industry) + '&location=' + encodeURIComponent(location);
            //};
            ...
            self.searchProjects = function (industry, location) {
                debugger;
                var self = this;
                ...
                var functionDone = function (hasError, message, data) {
                    if (hasError) {
                        ...
                        if (item) {
                            ...
                            debugger;
                            // Navigation to the CandidatesDetails page
                            window.location.href = "/IndexProjectDetails";
                        }
                ...
                var functionDone = function (hasError, message, data) {
                    if (hasError) {
                        ...
                        if (item) {
                            var name = item.ProjectName;
                            var category = item.ProjectCategory;
                            var start = item.ProjectStart;
                            var end = item.ProjectEnd;
                            var details = item.ProjectDetails;
                            var open = item.OpenPosition;
                            var work = item.WorkScope;
                            var post = item.PostAs;
            ...
            this.retrieveProjectDetailsFromLocalStorage = function () {
                ...
                if (window.location.pathname === "/Admin/ProjectDetails" || window.location.pathname === "/User/ProjectDetails" || window.location.pathname === "/IndexProjectDetails") {
