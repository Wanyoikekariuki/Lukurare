...
{
    public class CountryRepository : EFDatabaseModelBaseRepository<Country>
    {
        public CountryRepository(string currentModule)
            : base(currentModule) { }
        ...
        /// <returns></returns>
        public override Country CloneModel(Country model)
        {
            return new Country
            {
                Id = model.Id,
        ...
        public async override Task<ExecutionResult<Country>> Add(
            Country model,
        ...
        )
        {
            ...
            var executionResult = new ExecutionResult<Country>(true);
            try
            {
                ...
                context.Countries.Add(modelInsert);
        ...
        public async override Task<ExecutionResult<Country>> Delete(
            Country model,
            EFDatabaseModelDatabaseContext context = null
        )
        {
            ...
            try
            {
                ...
                context.Countries.Remove(DbInstance);
                if (createContextHere)
                {
                    await context.SaveChangesAsync();
        ...
        public async override Task<ExecutionResult<Country>> GetItem(
            int id,
            EFDatabaseModelDatabaseContext context = null
        )
        {
            //incase the context was null then mark this and use this into create a new context instance and later dispose it in finally
            var createContextHere = context == null;
            var executionResult = new ExecutionResult<Country>(true, "Entry found");
            try
            {
                if (createContextHere)
                    context = new EFDatabaseModelDatabaseContext();

                DbInstance = await context.Countries.FirstOrDefaultAsync(r => r.Id == id);
                if (DbInstance == null)
                {
        ...
        )
        {
            ...
            try
            {
                ...
                var filterSupporter = new FilterSupporter<Country>(countries, args);
        ...
        public async override Task<ExecutionResult<Country>> IsValidGeneral(
            Country model,
        ...
        )
        {
            ...
            return await Task.Run<ExecutionResult<Country>>(() =>
            {
                var result = new ExecutionResult<Country>(true);
                if (model == null)
                {
                    result.IsOkay = false;
                ...
                {
                    result.IsOkay = false;
                    result.Message = "Please supply the country name";
                    return result;
                }
                ...
                if (string.IsNullOrEmpty(model.CountryIcon))
                {
                    result.IsOkay = false;
                    result.Message = "Please supply the country icon";
                    return result;
                }
                if (string.IsNullOrEmpty(model.CountryIso))
                {
                    result.IsOkay = false;
                    result.Message = "Please supply the country iso";
                    return result;
                }
        ...
        public async override Task<ExecutionResult<Country>> IsValidInsert(
            Country model,
            EFDatabaseModelDatabaseContext context
        )
        {
            ...
            var countryAlreadyRegistered = await context.Countries
                .Where(
                    r => r.CountryCode == model.CountryCode || r.CountryName == model.CountryName
                )
                .FirstOrDefaultAsync();
            if (countryAlreadyRegistered != null)
            {
                result.IsOkay = false;
                if (countryAlreadyRegistered.CountryCode == model.CountryCode)
                    result.Message = $"The country code {model.CountryCode} is already used";
                else
                    result.Message = $"The country name {model.CountryName} is already used";
                return result;
            }
        ...
        public async override Task<ExecutionResult<Country>> IsValidUpdate(
            Country model,
            EFDatabaseModelDatabaseContext context
        )
        {
        ...
        public async override Task<ExecutionResult<Country>> Update(
            Country model,
            EFDatabaseModelDatabaseContext context = null
        )
        {
            ...
            try
            {
                ...
                DbInstance.CountryCode = model.CountryCode;
                DbInstance.CountryIcon = model.CountryIcon;
                DbInstance.CountryIso = model.CountryIso;
                DbInstance.CountryName = model.CountryName;
